...
export const useUIStore = defineStore('ui', () => {
  ...
  const sidebarExpanded = ref(
    savedSidebarExpanded === null ? true : savedSidebarExpanded === 'true',
  )
  // 默认只展开当前工作区的说明，避免六个入口长期占据侧栏高度。
  const workspaceListExpanded = ref(
    localStorage.getItem(WORKSPACE_LIST_STORAGE_KEY) === 'true',
  )
  ...
  const sidebarWidth = computed(() => {
    ...
    if (sidebarMode.value === 'full') {
      // Keep the content offset identical to the rendered sidebar width. The
      // previous 248/224 mismatch left a dead 24px strip beside every page.
      return 224
  ...
  const toggleWorkspaceList = () => {
    ...
    localStorage.setItem(
      ...
      String(workspaceListExpanded.value),
    )
  }
  ...
  const toggleTheme = () => {
    ...
    console.log(
      '[UIStore] document.documentElement data-theme 已设置为:',
      currentTheme.value,
    )
  }
